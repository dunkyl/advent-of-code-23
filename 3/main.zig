...
pub fn main() !void {
    ...
    var symbolsSet = std.AutoHashMap(u8, void).init(allocator);
    for (symbols) |c| try symbolsSet.put(c, {});
    ...
    var uniqueSpans = std.AutoHashMap(struct {Point, isize}, void).init(allocator);
    var sum: usize = 0;
    ...
    // const slice = schematic.sliceHAt(.{ .x = 2, .y = 2 }, 2).?;
    // print("{s}\n", .{slice});

    var spansIter = uniqueSpans.keyIterator();
    while (spansIter.next()) |span| {
        const sliceMaybe = schematic.sliceHAt(span[0], span[1]);
        if (sliceMaybe == null) {
            print("sliceMaybe == null at {d} {d} for {d}\n", .{span[0].x, span[0].y, span[1]});
            continue;
        }
